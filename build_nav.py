...
TARGET_FILE = "test.yml" # where to insert the nav (this is so I can use test.yml for testing)
DOCS_DIR_PATH = "docs" # location of your documentation
TAB_ORDER = ["index.md", "API Reference", "Handbook", "Concept Art"] # ordered list of your tabs
    ...
    for dirEntry in os.scandir(path):
        if dirEntry.is_dir():
            ...
            if file_name_tuple[1] == ".md":
                ...
                relative_file_path = dirEntry.path[path_substring_start_index:] # get rid of the root path at the start

                with open(dirEntry.path, "r", errors = "ignore") as f:
                    first_line = f.readline().strip()
                    if first_line[0:2] == "# ":
                        nav_files.append(f"{'  ' * depth}- \"{first_line[2:]}\": {relative_file_path}\n")
                    else:
                        nav_files.append(f"{'  ' * depth}- {file_name}: {relative_file_path}\n")
